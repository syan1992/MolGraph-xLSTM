...
class Experts(nn.Module):
    def __init__(self,
        ...
        w1 = torch.zeros(*num_experts, dim, output_dim)
        w1 = init_(w1)
        self.w1 = nn.Parameter(w1)

        w2 = torch.zeros(*num_experts, output_dim, output_dim)
        w2 = init_(w2)
        self.w2 = nn.Parameter(w2)

        w4 = torch.zeros(*num_experts, output_dim, output_dim)
        w4 = init_(w4)
        self.w4 = nn.Parameter(w4)
        
        w3 = torch.zeros(*num_experts, output_dim, output_dim)
        w3 = init_(w3)
    ...
    def forward(self, x):
        #freesolv, bbbp, bace, clintox
        hidden = torch.einsum('...nd,...dh->...nh', x, self.w1)
        hidden = self.act(hidden)

        #hidden = torch.einsum('...nd,...dh->...nh', hidden, self.w2)
        #hidden = self.act(hidden)

        #hidden = torch.einsum('...nd,...dh->...nh', hidden, self.w4)
        #hidden = self.act(hidden)
        
        out    = torch.einsum('...nh,...hd->...nd', hidden, self.w3)
...
class MoE(nn.Module):
    def __init__(self,
        ...
        hidden_dim = None,
        activation = nn.ReLU,
        second_policy_train = 'all', #'random',
        second_policy_eval = 'all',#'random',
        second_threshold_train = 0.2,
        second_threshold_eval = 0.2,
        capacity_factor_train = 1.25,
        capacity_factor_eval = 2.,
        ...
        self.experts = default(experts, lambda: Experts(dim, output_dim = output_dim, num_experts = num_experts, hidden_dim = hidden_dim, activation = activation))
        self.loss_coef = loss_coef
        self.output_dim = output_dim
    def forward(self, inputs, inputs_gate, **kwargs):
        b, n, d, e = *inputs.shape, self.num_experts
        dispatch_tensor, combine_tensor, loss = self.gate(inputs_gate)
        ...
        expert_outputs = expert_outputs.reshape([orig_shape[0], orig_shape[1], orig_shape[2], self.output_dim])#*orig_shape)

        output = torch.einsum('ebcd,bnec->bnd', expert_outputs, combine_tensor)
        #output = output / expert_inputs.shape[0]
